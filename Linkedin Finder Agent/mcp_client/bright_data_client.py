    ...
    try:
        ...
        def custom_handler(lp, context):
            ...
            if any(kw in err_lower for kw in ["post_writer", "remoteprotocolerror", "server disconnected"]):
                log_print("⚡ [MCP Transport] Server dropped idle connection stream (auto-reconnecting on demand).")
                return
...
async def reset_bright_data_tools():
    ...
    log_print("🔄 [MCP Client] Resetting cached Bright Data tools and client connection...")
    if _mcp_client is not None:
        try:
            ...
            log_print(f"⚠️ Error closing previous MCP client: {e}")
    _mcp_client = None
    _cached_tools = None
...
async def get_bright_data_tools(max_retries: int = 5, initial_backoff: float = 0.1, force_refresh: bool = False) -> List[BaseTool]:
    ...
    Includes thread/task-safe lock and retry logic with exponential backoff for transient errors.
    """
    global _mcp_client, _cached_tools
    ...
    async with _mcp_lock:
        # Check again after acquiring lock to avoid duplicate initializations
        if _cached_tools is not None and not force_refresh:
            return _cached_tools
            
        url = config.BRIGHT_DATA_MCP_URL
        if not url:
            log_print("MCP Failed: BRIGHT_DATA_MCP_URL is not set.")
        ...
        for attempt in range(1, max_retries + 1):
            try:
                log_print(f"Connecting to Bright Data MCP server (attempt {attempt}/{max_retries})...")
                ...
                if not tools:
                    log_print("MCP Failed: Server returned 0 tools.")
                    raise RuntimeError("MCP Failed: Server returned 0 tools.")
                
                _mcp_client = client
                _cached_tools = tools
                log_print(f"Successfully connected to Bright Data MCP server. Loaded {len(tools)} tools.")
                return tools
            except Exception as e:
                last_exception = e
                log_print(f"⚠️ Bright Data MCP connection attempt {attempt}/{max_retries} failed: {e}")
                if attempt < max_retries:
                    log_print(f"Retrying in {backoff}s...")
                    await asyncio.sleep(backoff)
                    backoff *= 2
                else:
                    log_print(f"MCP Failed: All {max_retries} connection attempts failed.")
